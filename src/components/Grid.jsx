...
export function Col({ span, className = '', children, as: Tag = 'div', ...rest }) {
  return (
    <Tag
      className={[span ? `ds-col-${span}` : 'ds-col', className].filter(Boolean).join(' ')}
      {...rest}
    >
      {children}
    </Tag>
...
Container.propTypes = { ...sharedProps, size: PropTypes.oneOf(['sm', 'md', 'lg', 'xl', '2xl', 'fluid']) };
Section.propTypes   = { ...sharedProps, size: PropTypes.oneOf(['sm', 'base', 'lg']) };
...
Stack.propTypes      = { ...sharedProps, gap: PropTypes.oneOf(['base', 'sm', 'lg', 'xl']) };
